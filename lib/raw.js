      ...
      async.forEachSeries(
        ...
        function (pdf_file, cb) {
          ...
          convert(pdf_file.file_path, quality, function (err, tif_path) {
            self.emit('log', 'converted page to intermediate tiff file, page '+ index+ ' (0-based indexing) of '+ num_pages);
            ...
            ocr(tif_path, ocr_flags, function (err, extract) {
              fs.unlink(tif_path, function (tif_cleanup_err, reply) {
                ...
                self.emit('log', 'raw ocr: page ' + page_number + ' of ' +num_pages + ' complete');
                single_page_pdf_file_paths.push(pdf_file.file_path);
                self.emit('page', { text: extract, index: index, num_pages: num_pages, pdf_path: pdf_path, single_page_pdf_path: pdf_file.file_path});
        ...
        function (err) {
          if (options.clean && split_output && split_output.folder) {
          ...
          if (!err) {
            self.emit('complete', { text_pages: text_pages, pdf_path: pdf_path, single_page_pdf_file_paths: single_page_pdf_file_paths});
            return;
          }
        });
    });
