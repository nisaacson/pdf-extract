 ...
 */
module.exports = function(input_path, ascii_only, callback) {
  // options is an optional parameter
  if (!callback || typeof callback != "function") {
    // callback must be the second parameter
    callback = ascii_only;
    ascii_only = true;
    // no option passed, use the default ASCII only character set for ocr
  ...
  fs.exists(input_path, function (exists) {
    ...
    var cmd = 'tesseract "'+input_path+'" "'+output_path+'" -psm 1 ';
    if (ascii_only) {
      cmd += 'alphanumeric'
    }
